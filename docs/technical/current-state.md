...
- Fase 0 (baseline/bescherming), Fase 1 (kritieke risico's) en het grootste deel van Fase 2 (quality gates) zijn uitgevoerd. Fase 2 leverde: een GitHub Actions CI-pipeline (`.github/workflows/ci.yml`) met verplichte gates (backend ruff + pytest, frontend `tsc` + build), een groene `ruff`-lint met `backend/pyproject.toml`, verwijdering van `ignoreBuildErrors` in de frontend (TypeScript wordt nu afgedwongen), en een frontend-tokenfix zodat de nu-beschermde endpoints werken voor ingelogde gebruikers. `mypy` draait informatief (baseline ~167 meldingen, aanscherpen in Fase 3). Openstaand in Fase 2: frontend ESLint-config. Daarna: Fase 3 (architectuur) en Fase 4 (deployment/observability). Bewust uitgesteld binnen Fase 1: transactiegrens bij multi-file ingest (productbeslissing) en een unique constraint op `ArtikelVoorraad` (vereist data-audit).
