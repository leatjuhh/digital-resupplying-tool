...
**Huidige toestand:** Voldoet grotendeels (2026-07/08). De twee grootste modules zijn in Fase 3 opgesplitst: `backend/routers/pdf_ingest.py` (was 913 r.) → HTTP-laag + `pdf_ingest_service.py` + `pdf_ingest_persistence.py` (Fase 3.1, PR-022); `backend/redistribution/algorithm.py` (was 880/922 r.) → `algorithm.py` (orchestratie, ~197 r.) + `data_loading.py` + `bundle_planner.py` + `legacy_size_planner.py` + `sequence.py` (Fase 3, PR-022), gedragsbehoudend geverifieerd via een oud-vs-nieuw output-hash over 1000 scenario's. Resterend: `backend/pdf_extract/pipeline.py` (528 r.) is nog niet gesplitst — openstaande, kleinere technische schuld. De norm wordt niet verzwakt om aan de huidige code te voldoen.
...
**Huidige toestand:** Voldoet gedeeltelijk (verbeterd 2026-08-06). `backend/db_models.py` bevat `UniqueConstraint`s (bv. regel 174 en 204, plus kolomniveau `unique=True` op meerdere plekken) en sinds PR-006 de eerste `CheckConstraint`s: `ArtikelVoorraad` heeft nu `ck_artikel_voorraad_voorraad_nonneg` en `ck_artikel_voorraad_verkocht_nonneg` (niet-negatieve aantallen, R5.4), met `backend/test_artikel_voorraad_constraint.py` als compenserende controle. Sinds PR-006-vervolg heeft `ArtikelVoorraad` ook een `UniqueConstraint` over (batch, volgnummer, filiaal, maat) — `uq_artikel_voorraad_batch_vlg_fil_maat` — tegen dubbele voorraadrijen, met nette per-bestand-foutafhandeling in de ingest (`backend/test_pdf_ingest_service.py::test_run_batch_ingest_duplicate_file_recovers`). NB: bij SQLite gelden deze vanaf tabel-aanmaak (verse DB); een bestaande tabel krijgt ze pas via een migratie (Fase 4). Resterend: `Proposal.moves` is nog een ongetypeerde `JSON`-kolom (`db_models.py`) zonder schemavalidatie op databaseniveau (zie Regel 9 en hoofdstuk 7). Dit is resterende technische schuld.
...
**Huidige toestand:** Voldoet (voor de twee eerder geregistreerde modules, opgelost in Fase 3.3, 2026-07-14 — zie hoofdstuk 15). `backend/redistribution/bv_config.py` en `backend/redistribution/store_profiles.py` hadden module-level *mutable* singletons (`_global_bv_config`, `_active_profiles`) met mutators (`reload_bv_config`, `set_store_profiles`). Die mutators zijn verwijderd; de configuratie wordt nu expliciet als parameter door het herverdelingsalgoritme doorgegeven (dependency injection, PR-020). De resterende BV-configuratie is een read-only, lazy geladen cache (BVConfig() doet file-I/O en wordt éénmalig per batch geresolved) en valt daarmee onder de in R6.1 toegestane read-mostly-configuratiecache-uitzondering; de winkelprofielen zijn een read-only constante. `backend/test_algorithm_config_injection.py` bewijst als compenserende controle dat een geïnjecteerde config van de algoritme-entrypoints tot in `validate_bv_move` doorwerkt. Voor de overige backend-modules is een uitputtende doorlichting niet uitgevoerd, maar er zijn geen nieuwe module-level mutable singletons geïntroduceerd.
...
**Automatische controle:** `ruff check`, `mypy`, `npm run lint`, `tsc --noEmit` en de frontend-build als CI-stappen. Huidige status (2026-08): ingevoerd in `.github/workflows/ci.yml`. Verplichte (blokkerende) gates: `ruff check .`, `mypy redistribution/` (Kritiek-domeinlaag, type-schoon), `pytest` (unit + security), frontend `tsc --noEmit`, `npm run lint` en `npm run build` (zonder `ignoreBuildErrors`). Niet-blokkerende baseline: `mypy .` over de volledige backend (de router-/legacy-laag heeft nog een baseline aan grotendeels SQLAlchemy-`Column` typefouten), die module-voor-module wordt opgeschoond en blokkerend gemaakt.
...
**Huidige toestand:** Voldoet grotendeels (2026-08). De oorspronkelijk grootste kloof is grotendeels gedicht: er is nu een CI-pipeline (`.github/workflows/ci.yml`) met `ruff`- (`pyproject.toml`) en `mypy`-configuratie, en `frontend/next.config.mjs` bevat géén `ignoreBuildErrors` meer — de frontend-build faalt nu bij typefouten. Blokkerende gates: `ruff check .`, `mypy redistribution/`, `pytest`, frontend `tsc --noEmit` + `npm run lint` + `npm run build`. Resterend: `mypy` over de volledige backend draait nog als niet-blokkerende baseline (router-/legacy-laag met een resterende baseline aan grotendeels SQLAlchemy-`Column` typefouten); dit wordt module-voor-module opgeschoond en blokkerend gemaakt. Dit is de laatste openstaande deelstap van Regel 10.
...
**Vastgestelde afwijking (opgelost in Fase 3.1, 2026-07-14):** `backend/routers/pdf_ingest.py` bevatte oorspronkelijk (913 regels) aanzienlijke domeinlogica rechtstreeks in de routerlaag — parsing-orchestratie en het toepassen van moves op voorraad. In Fase 3.1 (PR-022) is dit gesplitst conform de lagen uit dit hoofdstuk: de domeinlogica staat nu in `backend/pdf_ingest_service.py` (o.a. `run_batch_ingest`, `apply_moves_to_inventory`, `build_proposal_rows`, `generate_and_save_proposals`) en de persistentie in `backend/pdf_ingest_persistence.py` (`save_parsed_records`, `save_generated_proposals`). `routers/pdf_ingest.py` is teruggebracht tot een dunne HTTP-laag (598 regels, uitsluitend route-handlers, request/response-mapping en auth) en voldoet daarmee aan R4.4. Het externe API-contract is ongewijzigd; het regressievangnet staat in `backend/test_pdf_ingest_service.py`. *(Het aanmaken van `Feedback`-records bij approve/reject/edit blijft bewust in de mutatie-handlers, omdat dat een dunne, endpoint-specifieke schrijfstap is.)*
...
**Databaseconstraints:** `backend/db_models.py` bevat diverse `unique=True`-kolommen en twee `UniqueConstraint`-definities (`uq_assignment_series_batch_store` op regel 174, `uq_assignment_item_proposal_route` op regel 204). Sinds PR-006 zijn er ook `CheckConstraint`s voor niet-negatieve aantallen op `ArtikelVoorraad` (`voorraad >= 0`, `verkocht >= 0`) én een `UniqueConstraint` over (batch, volgnummer, filiaal, maat) (`uq_artikel_voorraad_batch_vlg_fil_maat`): een dubbele ingest binnen dezelfde batch levert daardoor geen dubbele voorraadrijen (en dus geen dubbeltelling) meer op. De ingest herstelt de sessie bij een schending en markeert het betreffende bestand als `FAILED` (per-bestand-isolatie blijft behouden). NB (SQLite): geldt vanaf tabel-aanmaak op een verse DB; een bestaande tabel krijgt de constraint pas via een migratie (Fase 4). Omdat elke upload een nieuwe `batch_id` krijgt, dekt dit duplicaten *binnen* één batch; cross-batch-idempotentie (dezelfde PDF opnieuw uploaden) blijft een apart aandachtspunt.
...
**Audit trail:** `Proposal` heeft sinds de review-audit-stap naast `reviewed_at` nu ook een `reviewed_by`-kolom (`db_models.py`): de approve-, reject- en edit-handlers leggen de `username` van de ingelogde reviewer vast (`backend/test_proposal_review_audit.py` als compenserende controle). Wie welk voorstel goedkeurde/afkeurde is daarmee **wel** herleidbaar. Aanvullend leggen `Feedback`-records nu ook een `user_id`-verwijzing (FK naar `users`) vast: de approve/reject/edit-handlers zetten `user_id` van de ingelogde gebruiker, zodat ook per-mutatie herleidbaar is wie de actie deed (`backend/test_proposal_review_audit.py` borgt dit).
...
**Auditlogging voor mutaties:** wie welk voorstel goedkeurde of afkeurde MOET herleidbaar zijn. Dit is nu vastgelegd: `Proposal.reviewed_by` bevat de `username` van de reviewer, en `Feedback.user_id` (FK naar `users`) legt per mutatie de uitvoerende gebruiker vast — beide gezet in de approve/reject/edit-handlers (zie hoofdstuk 6).
...
|---|---|---|---|---|---|
| 2026-07-13 (~~afwijking~~ **opgelost 2026-07-14**) | R6.1 | `backend/redistribution/bv_config.py` (`_global_bv_config`) | Module-level mutable singleton voor BV-configuratie | **Opgelost in Fase 3.3 (PR-020)**: de mutator `reload_bv_config()` is verwijderd en de config wordt nu expliciet als parameter door het algoritme doorgegeven (dependency injection). De resterende lazy singleton is read-only en valt onder de R6.1-cache-uitzondering. Compenserende controle: `backend/test_algorithm_config_injection.py` bewijst dat een geïnjecteerde config doorpropageert | Projecteigenaar |
| 2026-07-13 (~~afwijking~~ **opgelost 2026-07-14**) | R6.1 | `backend/redistribution/store_profiles.py` (`_active_profiles`) | Module-level mutable singleton voor winkelprofielen, gemuteerd via `set_store_profiles()` | **Opgelost in Fase 3.3 (PR-020)**: de mutable global + setter zijn verwijderd; `_DEFAULT_PROFILES` is read-only en afwijkende profielen worden via de `profiles`-parameter geïnjecteerd. Compenserende controle: `test_algorithm_config_injection.py` | Projecteigenaar |
| 2026-07-13 (~~afwijking~~ **opgelost 2026-07-14**) | R4.4 | `backend/routers/pdf_ingest.py` | Router mengde HTTP-routing, domeinlogica en persistentie (oorspronkelijk 913 regels) | **Opgelost in Fase 3.1 (PR-022)**: domein → `pdf_ingest_service.py`, persistentie → `pdf_ingest_persistence.py`; router teruggebracht tot dunne HTTP-laag (598 r.). Compenserende controle: `backend/test_pdf_ingest_service.py` in de CI-gate | Projecteigenaar |
| 2026-07-13 (~~afwijking~~ **opgelost 2026-08-06**) | R4.1–R4.3 | `backend/redistribution/algorithm.py` (was 880/922 regels) | Grote, samenhangende domeinmodule voor het herverdelingsalgoritme | **Opgelost in Fase 3 (PR-022)**: gesplitst in `algorithm.py` (orchestratie, ~197 r.), `data_loading.py`, `bundle_planner.py`, `legacy_size_planner.py` en `sequence.py`. Compenserende controle: de 25+ invariant-/bundle-/injectie-tests plus een oud-vs-nieuw output-hash over 1000 scenario's (bit-identiek) | Projecteigenaar |
| 2026-07-13 | R6.1 | `backend/rate_limit.py` (`_login_rate_limiter`) | Nieuw: de login-rate-limiter (PR-013) houdt bewust procesbrede, muteerbare state (mislukte-login-tellers) bij — inherent aan een rate limiter. Valt onder de R6.1-cache-uitzondering (expliciete expiry-invalidatie + `threading.Lock` + begrensd geheugen), en wordt via dependency injection aangeboden i.p.v. als vrije globale | **Aanwezig**: `backend/test_rate_limit.py` (10 deterministische unit-tests) + integratietest in `test_security_hardening.py` (429 + `Retry-After`), beide in de CI-gate | Projecteigenaar |

De eerste vier rijen zijn bij het opstellen van dit document geregistreerd als reeds bestaande, bewuste afwijkingen (technische schuld), niet als nieuw goedgekeurde uitzonderingen voor toekomstige code. De vijfde rij (R6.1, `rate_limit.py`) is het eerste voorbeeld van een **nieuw** ontstane afwijking die conform dit proces is geregistreerd, mét een concrete compenserende controle. Nieuwe afwijkingen die vanaf nu ontstaan MOETEN op dezelfde wijze geregistreerd worden, met een concrete compenserende controle — het enkel noteren van de afwijking zonder compensatie volstaat niet voor nieuwe code.
