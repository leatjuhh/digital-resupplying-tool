...
**Audit trail:** `Proposal` heeft sinds de review-audit-stap naast `reviewed_at` nu ook een `reviewed_by`-kolom (`db_models.py`): de approve-, reject- en edit-handlers leggen de `username` van de ingelogde reviewer vast (`backend/test_proposal_review_audit.py` als compenserende controle). Wie welk voorstel goedkeurde/afkeurde is daarmee **wel** herleidbaar. Resterend: `Feedback`-records leggen `category`/`action_taken` per move vast maar bevatten nog geen `user_id`-verwijzing naar de uitvoerende gebruiker — een kleiner, apart aandachtspunt.
...
**Auditlogging voor mutaties:** wie welk voorstel goedkeurde of afkeurde MOET herleidbaar zijn. Dit is nu vastgelegd: `Proposal.reviewed_by` bevat de `username` van de reviewer, gezet in de approve/reject/edit-handlers (zie hoofdstuk 6). Resterend, kleiner punt: `Feedback` heeft nog geen `user_id`.
