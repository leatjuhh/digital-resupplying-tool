...
**Huidige toestand:** Voldoet gedeeltelijk (verbeterd 2026-08-06). `backend/db_models.py` bevat `UniqueConstraint`s (bv. regel 174 en 204, plus kolomniveau `unique=True` op meerdere plekken) en sinds PR-006 de eerste `CheckConstraint`s: `ArtikelVoorraad` heeft nu `ck_artikel_voorraad_voorraad_nonneg` en `ck_artikel_voorraad_verkocht_nonneg` (niet-negatieve aantallen, R5.4), met `backend/test_artikel_voorraad_constraint.py` als compenserende controle. NB: bij SQLite gelden deze vanaf tabel-aanmaak (verse DB); een bestaande tabel krijgt ze pas via een migratie (Fase 4). Resterend: `ArtikelVoorraad` heeft nog geen unieke constraint over batch/artikel/filiaal/maat (bewuste vervolgstap, met nette ingest-foutafhandeling), en `Proposal.moves` is nog een ongetypeerde `JSON`-kolom (`db_models.py`) zonder schemavalidatie op databaseniveau (zie Regel 9 en hoofdstuk 7). Dit is resterende technische schuld.
...
**Databaseconstraints:** `backend/db_models.py` bevat diverse `unique=True`-kolommen en twee `UniqueConstraint`-definities (`uq_assignment_series_batch_store` op regel 174, `uq_assignment_item_proposal_route` op regel 204). Sinds PR-006 zijn er ook `CheckConstraint`s voor niet-negatieve aantallen op `ArtikelVoorraad` (`voorraad >= 0`, `verkocht >= 0`). Resterend: `ArtikelVoorraad` heeft nog geen unieke constraint over de combinatie batch/artikel/filiaal/maat — een herhaalde ingest van dezelfde PDF kan daardoor nog dubbele voorraadrijen opleveren (bewuste vervolgstap: unieke constraint + idempotente/nette ingest-foutafhandeling).
